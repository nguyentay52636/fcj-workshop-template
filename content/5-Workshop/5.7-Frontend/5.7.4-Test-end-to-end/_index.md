...
| --- | ---------------------------------------------------------------- | ------------------------------------------------------------------- | ------ |
| 1   | Login with a valid Cognito user                                  | Receive `IdToken`; auth indicator turns green; Upload / Ask enabled | Pass — token received, controls enabled |
| 2   | Login with wrong password                                        | Clear error; UI does not crash; buttons stay disabled               | Pass — error shown, buttons remained disabled |
| 3   | Upload `.txt` / `.md` (text)                                     | Content appears in the editor and can be edited before send         | Pass — text editable in the editor |
...
| 5   | Upload a scanned PDF (no text) — choose **Yes**                  | OCR confirm dialog → Textract → pipeline continues                  | Pass — dialog shown; Textract path continued |
| 6   | Upload a scanned PDF — choose **No**                             | Stops as cancelled; no Textract cost                                | Pass — cancelled status; no Textract call |
| 7   | Ask about the document just uploaded                             | Grounded answer; source file tags when present                      | Pass — answer grounded with source tags |
